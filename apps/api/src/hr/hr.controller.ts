...
export class HrController {
  ...
  createDepartment(@CurrentUser() user: any, @Body() body: { name: string; description?: string; location?: string }) {
    return this.hrService.createDepartment(user.organizationId, body.name, body.description, body.location);
  }

  @Patch('departments/:id')
  @Roles('HR_ADMIN', 'COMPANY_ADMIN')
  @ApiOperation({ summary: 'Update department' })
  patchDepartment(@CurrentUser() user: any, @Param('id') id: string, @Body() body: { name: string; description?: string; location?: string }) {
    return this.hrService.updateDepartment(id, user.organizationId, body.name, body.description, body.location);
  }
  ...
  updateRole(@CurrentUser() user: any, @Param('id') id: string, @Body('role') role: string) {
  ...
  moveUserDepartment(@CurrentUser() user: any, @Param('id') id: string, @Body('departmentId') departmentId: string | null) {
    return this.hrService.updateUserDepartment(id, user.organizationId, departmentId ?? null);
  }
  ...
  getAuditLogs(@CurrentUser() user: any, @Query('limit') limit?: string) {
    return this.hrService.getAuditLogs(user.organizationId, limit ? parseInt(limit) : 100);
  }
}
