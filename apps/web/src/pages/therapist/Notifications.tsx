...
import { Bell, TrendingUp, UserPlus, CheckCircle, AlertTriangle, Shield, Activity, Trash2 } from 'lucide-react';
...
const TYPE_ICON: Record<string, { icon: any; bg: string; color: string }> = {
  PAIN_ESCALATION:      { icon: TrendingUp,    bg: 'bg-red-50',    color: 'text-red-600' },
  CHECKIN_REMINDER:     { icon: Activity,      bg: 'bg-brand-50',  color: 'text-brand-600' },
  PROGRAM_ASSIGNED:     { icon: UserPlus,      bg: 'bg-blue-50',   color: 'text-blue-600' },
  INCIDENT_UPDATE:      { icon: AlertTriangle, bg: 'bg-orange-50', color: 'text-orange-600' },
  RTW_MILESTONE:        { icon: CheckCircle,   bg: 'bg-green-50',  color: 'text-green-600' },
  RISK_THRESHOLD_BREACH:{ icon: Shield,        bg: 'bg-red-50',    color: 'text-red-600' },
    ...
    <div className="space-y-6">
      ...
      {notifications.length === 0 ? (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center py-16">
          ...
          <p className="text-gray-400 text-sm mt-1">Escalation alerts and program updates will appear here.</p>
        </motion.div>
      ) : (
        <div className="space-y-3">
          {notifications.map((notif: any, i: number) => {
            ...
            return (
              <motion.div key={notif.id} initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.04 }}>
                <Card
                  className={`transition-all cursor-pointer ${notif.isRead ? 'opacity-60' : 'border-brand-100 shadow-sm'}`}
