...
import { ShieldCheck, Eye, EyeOff, AlertCircle, KeyRound, Building2 } from 'lucide-react';
...
export default function Login() {
  ...
  useEffect(() => {
    ...
    if (err === 'sso_failed') setError('SSO sign-in failed. Contact your administrator.');

    // Handle MFA redirect from Google/SSO
    const mfa = searchParams.get('mfaRequired');
    const ct = searchParams.get('challengeToken');
    if (mfa === 'true' && ct) {
      setMfaRequired(true);
  ...
  const onSubmit = async (data: LoginForm) => {
    ...
    try {
      ...
      navigate(!user.isOnboarded && user.role === 'WORKER' ? '/onboarding' : getDashboardPath(user.role));
    } catch (err: any) {
  ...
  const onMfaSubmit = async () => {
    ...
    setError('');
    try {
      const res = await api.post('/auth/mfa/verify', { challengeToken, token: totpCode });
      ...
      <div className="hidden lg:flex lg:w-1/2 gradient-brand flex-col justify-between p-12 relative overflow-hidden">
        ...
        <div className="relative">
          <blockquote className="text-white/90 text-2xl font-medium leading-relaxed mb-6">
            "WorkSafe reduced our MSK injury rate by 34% in the first year. The risk intelligence dashboard paid for itself in one quarter."
          </blockquote>
        ...
        <div className="relative grid grid-cols-3 gap-4">
          {[{ value: '247', label: 'Active workers' }, { value: '84%', label: 'Check-in rate' }, { value: '34', label: 'Risk score' }].map((s) => (
            <div key={s.label} className="bg-white/10 rounded-xl p-4 text-white">
            ...
            {mfaRequired ? (
              ...
              <motion.div key="mfa" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
                <div className="mb-8">
                  ...
                  <p className="text-gray-600 mt-1 text-sm">Enter the 6-digit code from your authenticator app.</p>
                </div>
                ...
                <div className="space-y-4">
                  ...
                  <Button className="w-full" size="lg" onClick={onMfaSubmit} disabled={totpCode.length !== 6 || mfaLoading}>
                    {mfaLoading ? 'Verifying…' : 'Verify'}
                  </Button>
                  <button onClick={() => { setMfaRequired(false); setError(''); setTotpCode(''); }} className="w-full text-sm text-gray-500 hover:text-gray-700">
                    ← Back to sign in
                  </button>
                </div>
              </motion.div>
              ...
              <motion.div key="login" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }}>
                ...
                {error && (
                  <motion.div className="mb-4 p-3 bg-red-50 border border-red-200 rounded-lg flex items-center gap-2 text-red-700 text-sm" initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }}>
                    <AlertCircle className="w-4 h-4 shrink-0" />{error}
                  </motion.div>
                )}
                ...
                <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <div className="relative">
                      <Input id="password" type={showPassword ? 'text' : 'password'} placeholder="••••••••" {...register('password')} className={errors.password ? 'border-red-500 pr-10' : 'pr-10'} />
                  ...
                  <Button type="submit" variant="brand" size="lg" className="w-full" loading={isSubmitting}>Sign in</Button>
                </form>
                ...
                <div className="space-y-2">
                  <Button variant="outline" size="lg" className="w-full" onClick={() => window.location.href = `${API_BASE}/auth/google`}>
                  ...
                  <Button variant="outline" size="lg" className="w-full" onClick={() => window.location.href = `${API_BASE}/auth/saml`}>
